    ...
    <Section id="book-a-call">
      <Reveal className="glass grid grid-cols-12 items-start gap-6 rounded-[28px] p-6 md:gap-12 md:p-14">
        <div className="col-span-12 flex flex-col gap-4 md:col-span-5 md:gap-[22px]">
          ...
          <p className="text-[17px] leading-[1.65] text-white/74 md:text-lg">{bookCall.body}</p>
          {email ? (
            <div className="flex flex-col gap-1.5 md:pt-2">
              <span className="text-sm text-white/55">{bookCall.preferEmail}</span>
              <a
                href={`mailto:${email}`}
                className="inline-flex min-h-11 items-center text-[17px] font-bold text-teal hover:text-white"
              >
                {email}
              </a>
            </div>
